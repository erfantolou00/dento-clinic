...
export async function POST(request: Request) {
  try {
    ...
    if (!appointmentNotificationEmail) {
      console.error(
        "APPOINTMENT_NOTIFICATION_EMAIL is not configured.",
      );
    ...
    if (!appointmentFromEmail) {
      console.error("APPOINTMENT_FROM_EMAIL is not configured.");
    ...
    const { error } = await resend.emails.send({
      ...
      html: buildAppointmentEmail(appointment),
    });
